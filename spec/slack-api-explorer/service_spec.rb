...
describe SlackApiExplorer::Service do
  let(:team) { Fabricate(:team) }
  let(:server) { SlackApiExplorer::Server.new(team: team) }
  let(:services) { SlackApiExplorer::Service.instance_variable_get(:@services) }
  before do
    allow(SlackApiExplorer::Server).to receive(:new).with(team: team).and_return(server)
    allow(EM).to receive(:next_tick).and_yield
    allow(EM).to receive(:defer).and_yield
    allow(server).to receive(:stop!)
  end
  after do
    SlackApiExplorer::Service.reset!
  end
  it 'starts a team' do
  ...
  context 'started team' do
    ...
    it 'removes team service' do
      SlackApiExplorer::Service.stop!(team)
      expect(services.size).to eq 0
    end
    it 'deactivates a team' do
      SlackApiExplorer::Service.deactivate!(team)
